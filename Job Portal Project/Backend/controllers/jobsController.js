...
exports.createJob = async (req, res, next) => {
    try {
        const job = await Job.create({
            ...
            jobType: req.body.jobType, // For job type category
            user: req.user.id
        });
        res.status(201).json({
            success: true,
...
exports.showJobs = async (req, res, next) => {
    ...
    // For filtering jobs by category ids
    let ids = [];
    ...
    // For Printing Ids condition
    let categ = cat !== '' ? cat : ids;

    // To get jobs by location
    let locations = [];
    const jobByLocation = await Job.find({}, { location: 1 });
    jobByLocation.forEach(val => {
        locations.push(val.location);
    });
    // To print unique location (to avoid duplicates)
    ...
    // Sort jobs by date in decending order
    try {
